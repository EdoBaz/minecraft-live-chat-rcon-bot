@echo off
title ProgettoBOT Startup (YouTube)
...
:: 2) Start YouTube webhook/polling
start "YT Bot" cmd /k ^
  "cd /d \"%~dp0yt-chat\" && ^
  \"%PYTHON_BIN%\" yt_chat_bot.py"
...
:: 4) Start the block progress monitor
start "YT BlockProgress" cmd /k ^
  "cd /d \"%~dp0yt-chat\" && ^
  \"%PYTHON_BIN%\" block_progress.py"

echo YouTube mode started.
pause
